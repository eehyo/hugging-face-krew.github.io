...
1. **score_mod를 추가하는 방법**은 어텐션이 비활성화된 위치(즉, 주의를 기울이지 않아야 하는 위치)에 causal mask에 score_mod를 추가하는 방식입니다. 그러나 단순히 스칼라 값을 더하는 것만으로도 **FlexAttention의 성능이 크게 저하**되었습니다. 그 이유는, 우리의 경우 score_mod가 최적화된 CUDA 커널 외부에서 더해지기 때문입니다.
